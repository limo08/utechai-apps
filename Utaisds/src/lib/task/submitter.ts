...
import { isUtKeyConfigured } from '@/lib/utkey-pool'
...
export async function submitTask(params: {
  ...
  // UT-KEY 检查：计费任务必须先绑定 UT-KEY（支持多 KEY）
  if (isBillableTaskType(params.type)) {
    const configured = await isUtKeyConfigured(params.userId)
    if (!configured) {
      throw new ApiError('UT_KEY_NOT_CONFIGURED', {
        detail: '请先在设置中心配置 UT-KEY 才能使用生成功能',
      })
    }
  }
