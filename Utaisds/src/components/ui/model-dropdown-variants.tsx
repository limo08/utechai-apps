            ...
            >
                <div className="flex items-center gap-2 flex-1 min-w-0">
                    ...
                    {activeModel?.providerName && (
                        <span className="text-xs font-medium px-2 py-0.5 rounded-full bg-black/5 dark:bg-white/10 text-[var(--glass-text-secondary)]">
                            {activeModel.providerName}
                        </span>
                    )}
                    {summary && <span className="text-sm text-[var(--glass-text-tertiary)] ml-auto pr-2">{summary}</span>}
                </div>
                            ...
                            return (
                                ...
                                >
                                    <div className="flex items-center gap-2">
                                        ...
                                        <span className={`text-xs px-1.5 py-0.5 rounded-md ${active ? 'bg-white/20 text-white dark:bg-black/10 dark:text-black' : 'border border-[var(--glass-stroke-base)] text-[var(--glass-text-tertiary)]'}`}>
                                            {m.providerName}
                                        </span>
                                    </div>
                            ...
                            {props.capabilityFields.map(field => {
                                ...
                                return (
                                    <div key={field.field} className="flex items-center justify-between">
                                        <span className="text-sm font-semibold text-[var(--glass-text-secondary)]">{field.label || field.field}</span>
                                            ...
                                            {field.options.map((opt) => {
                                                ...
                                                return (
                                                    <button
                                                        ...
                                                        className={`px-3 py-1 text-sm font-medium rounded-[8px] transition-all ${active ? 'bg-white dark:bg-[#333] shadow-sm text-[var(--glass-text-primary)]' : 'text-[var(--glass-text-tertiary)] hover:text-[var(--glass-text-secondary)]'}`}
                                                    >
                                                        {s}
                                                    </button>
                                                )
                    ...
                    <div className="overflow-y-auto max-h-[200px]">
                        {props.models.map(m => {
                            ...
                            return (
                                ...
                                >
                                    <div className="flex items-center gap-2">
                                        <span className={`text-sm ${active ? 'text-[var(--glass-text-primary)] font-medium' : 'text-[var(--glass-text-secondary)]'}`}>{m.label}</span>
                                        {m.providerName && (
                                            <span className="text-xs text-[var(--glass-text-tertiary)]">{m.providerName}</span>
                                        )}
                                    </div>
                            ...
                            {props.capabilityFields.map(field => {
                                ...
                                return (
                                    <div key={field.field} className="flex flex-col border-b last:border-0 border-[var(--glass-stroke-subtle)]">
                                        <div className="px-3 pt-2 text-xs tracking-wider uppercase text-[var(--glass-text-tertiary)] font-semibold">{field.label || field.field}</div>
                                            ...
                                            {field.options.map((opt) => {
                                                ...
                                                return (
                                                    <button
                                                        key={s}
                                                        onClick={() => props.onCapabilityChange(field.field, s, field.options[0])}
                                                        className={`min-w-[40px] px-2 py-1 text-xs font-mono rounded transition-colors ${active ? 'bg-[var(--glass-text-primary)] text-[var(--glass-bg-base)]' : 'bg-transparent text-[var(--glass-text-secondary)] hover:bg-[var(--glass-bg-hover)] box-border border border-transparent'}`}
                                                    >
                                                        {s}
                                                    </button>
                                                )
            ...
            >
                ...
                <div className="relative flex flex-col items-start min-w-0 pr-4 z-10">
                    <div className="flex items-center gap-2 w-full">
                        ...
                        {activeModel?.providerName && (
                            <span className="px-1.5 py-0.5 rounded border border-[var(--glass-stroke-base)] text-xs text-[var(--glass-text-tertiary)] uppercase tracking-wider">
                                {activeModel.providerName}
                            </span>
                        )}
                    </div>
                    {summary && <span className="text-sm mt-0.5 font-medium text-blue-500 dark:text-blue-400 opacity-80">{summary}</span>}
                </div>
                            ...
                            {props.capabilityFields.map(field => {
                                ...
                                return (
                                    <div key={field.field} className="flex flex-col gap-2">
                                        <div className="text-sm font-semibold text-[var(--glass-text-primary)]">{field.label || field.field}</div>
                                            ...
                                            {field.options.map((opt) => {
                                                ...
                                                return (
                                                    <button
                                                        key={s}
                                                        onClick={() => props.onCapabilityChange(field.field, s, field.options[0])}
                                                        className={`px-4 py-1.5 rounded-full text-sm font-bold transition-all border ${active ? 'bg-blue-500/10 border-blue-500 text-blue-600 dark:text-blue-400' : 'bg-transparent border-[var(--glass-stroke-base)] text-[var(--glass-text-secondary)] hover:border-[var(--glass-text-tertiary)]'}`}
                                                    >
                                                        {s}
                                                    </button>
                                                )
        ...
        <>
            ...
            >
                <div className="flex items-center gap-3">
                    ...
                    {activeModel?.providerName && (
                        <span className="px-2 py-0.5 rounded-[6px] border border-[var(--glass-stroke-subtle)] text-xs text-[var(--glass-text-secondary)] bg-[var(--glass-bg-base)] shadow-sm">
                            {activeModel.providerName}
                        </span>
                    )}
                </div>
                <div className="flex items-center gap-3">
                    {summary && <span className="font-medium text-sm text-[var(--glass-text-secondary)]">{summary}</span>}
                    <AppIcon name="chevronDown" className={`w-4 h-4 text-[var(--glass-text-tertiary)] transition-transform ${isOpen ? 'rotate-180' : ''}`} />
                </div>
            </button>
            ...
            {isOpen && createPortal(
                <div ref={panelRef} style={panelStyle} className="glass-surface-modal rounded-[16px] shadow-[0_16px_50px_rgba(0,0,0,0.12)] border border-[var(--glass-stroke-base)] overflow-hidden flex flex-col bg-white dark:bg-[#1C1C1E]">
                    ...
                    <div className="px-3 pt-3 pb-2 bg-[var(--glass-bg-base)]">
                        <div className="text-sm font-bold text-[var(--glass-text-secondary)] mb-2 px-1">选择模型</div>
                            ...
                            {props.models.map(m => {
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        {m.providerName && (
                                            <span className="px-1.5 py-0.5 rounded text-xs bg-black/5 dark:bg-white/10 text-[var(--glass-text-secondary)] ml-2">{m.providerName}</span>
                                        )}
                    ...
                    {props.capabilityFields.length > 0 && (
                        <div className="p-4 bg-[var(--glass-bg-surface)] border-t border-[var(--glass-stroke-subtle)] space-y-4">
                            <div className="text-sm font-bold text-[var(--glass-text-secondary)]">参数配置</div>
                            {props.capabilityFields.map(field => {
                                const val = props.capabilityOverrides[field.field] !== undefined ? String(props.capabilityOverrides[field.field]) : String(field.options[0] || '')
                                        ...
                                        {useSelectBox ? (
                                            <div className="relative w-[120px]">
                                                <select
                                                    ...
                                                    className="w-full h-[36px] appearance-none bg-[var(--glass-bg-base)] border border-[var(--glass-stroke-base)] rounded-[8px] px-3 font-medium text-sm text-[var(--glass-text-primary)] focus:outline-none focus:border-blue-500"
                                                >
                                                ...
                                                {field.options.map((opt) => {
                                                    ...
                                                    return (
                                                        <button
                                                            key={s}
                                                            onClick={() => props.onCapabilityChange(field.field, s, field.options[0])}
                                                            className={`px-3 py-1.5 text-sm font-medium rounded-[7px] transition-all min-w-[50px] text-center ${active ? 'bg-blue-100 dark:bg-blue-900/50 text-blue-700 dark:text-blue-300 shadow-sm' : 'text-[var(--glass-text-secondary)] hover:text-[var(--glass-text-primary)] hover:bg-black/5 dark:hover:bg-white/5'}`}
                                                        >
                                                            {s}
                                                        </button>
                                                    )
        ...
        <>
            ...
            >
                ...
                <div className="flex items-center gap-2 mt-1 opacity-70">
                    <span className="text-sm text-[var(--glass-text-tertiary)] font-mono uppercase">
                        {activeModel?.providerName || 'MODEL'}
                    </span>
                    {summary && (
                        <>
                            <span className="w-1 h-1 rounded-full bg-gray-400" />
                            <span className="text-sm text-[var(--glass-text-secondary)]">{summary}</span>
                        </>
                    )}
                </div>
            </button>
                        ...
                        {props.models.map(m => {
                            ...
                            return (
                                ...
                                >
                                    ...
                                    {m.providerName && (
                                        <span className="text-xs font-mono text-[var(--glass-text-tertiary)] bg-[var(--glass-bg-surface)] px-2 py-0.5 rounded">{m.providerName}</span>
                                    )}
                                </button>
                            )
                        })}
                    ...
                    {props.capabilityFields.length > 0 && (
                        <div className="p-4 bg-[var(--glass-bg-surface-strong)] border-t border-[var(--glass-stroke-base)] space-y-4">
                            {props.capabilityFields.map(field => {
                                ...
                                return (
                                    <div key={field.field} className="flex flex-col gap-2">
                                        <span className="text-xs uppercase tracking-widest font-bold text-[var(--glass-text-tertiary)]">{field.label || field.field}</span>
                                            ...
                                            {field.options.map((opt) => {
                                                ...
                                                return (
                                                    <button
                                                        key={s}
                                                        onClick={() => props.onCapabilityChange(field.field, s, field.options[0])}
                                                        className={`flex-1 py-1.5 text-sm font-semibold rounded-md border-b-2 transition-all ${active ? 'border-[#FCA5A5] text-[#FCA5A5] bg-[#FCA5A5]/5' : 'border-transparent text-[var(--glass-text-secondary)] hover:bg-[var(--glass-bg-hover)]'}`}
                                                    >
                                                        {s}
                                                    </button>
                                                )
