        ...
        <div className="flex items-center bg-[var(--glass-bg-surface)] border border-[var(--glass-stroke-base)] rounded-xl shadow-sm backdrop-blur-md">
            ...
            >
                <div className="flex flex-col items-start min-w-0 pr-2">
                    <span className="text-xs font-bold text-[var(--glass-text-tertiary)] uppercase tracking-wider mb-0.5">模型 Model</span>
            ...
            >
                <div className="flex flex-col items-start min-w-0 pr-2">
                    <span className="text-xs font-bold text-[var(--glass-text-tertiary)] uppercase tracking-wider mb-0.5">参数 Params</span>
            ...
            {paramOpen && props.capabilityFields.length > 0 && createPortal(
                <div ref={paramPanel} style={paramStyle} className="glass-surface-modal rounded-xl shadow-lg border border-[var(--glass-stroke-base)] p-4 space-y-4">
                    {props.capabilityFields.map(field => {
                        ...
                        return (
                            <div key={field.field}>
                                <div className="text-sm font-medium text-[var(--glass-text-secondary)] mb-2">{field.label || field.field}</div>
                                    ...
                                    {field.options.map(opt => {
                                        ...
                                        return (
                                            <button
                                                ...
                                                className={`flex-1 px-2 py-1.5 text-sm rounded-lg transition-colors border ${active ? 'bg-blue-50/50 dark:bg-blue-900/30 border-blue-500 text-blue-600 dark:text-blue-400 font-semibold' : 'bg-transparent border-[var(--glass-stroke-base)] text-[var(--glass-text-secondary)] hover:hover:bg-[var(--glass-bg-hover)]'}`}
                                            >
                                                {s}
                                            </button>
                                        )
        ...
        <div className="bg-[var(--glass-bg-surface-strong)] rounded-2xl border border-[var(--glass-stroke-subtle)] overflow-hidden transition-all duration-300 shadow-sm">
            ...
            >
                <div className="flex items-center gap-4">
                    ...
                    <div className="text-left flex flex-col">
                        ...
                        <span className="text-sm text-[var(--glass-text-tertiary)] mt-0.5">
                            展开以修改模型或参数设置
                        </span>
                    </div>
                </div>
            ...
            <div className={`transition-all duration-300 ease-in-out ${isExpanded ? 'max-h-[800px] opacity-100' : 'max-h-0 opacity-0'} overflow-hidden`}>
                <div className="p-4 pt-0 border-t border-[var(--glass-stroke-base)] mt-2 mx-4">
                    <div className="mt-4 mb-2 text-sm font-bold uppercase tracking-wider text-[var(--glass-text-secondary)]">1. 选择模型</div>
                    <div className="grid grid-cols-2 gap-2 mb-6">
                        {props.models.map(m => {
                            ...
                            return (
                                ...
                                >
                                    <div className={`text-sm font-semibold mb-1 ${active ? 'text-blue-600 dark:text-blue-400' : 'text-[var(--glass-text-primary)]'}`}>{m.label}</div>
                                    {m.providerName && <div className="text-xs text-[var(--glass-text-tertiary)]">{m.providerName}</div>}
                                </button>
                            )
                        })}
                    </div>
                    ...
                    {props.capabilityFields.length > 0 && (
                        <>
                            <div className="mb-2 text-sm font-bold uppercase tracking-wider text-[var(--glass-text-secondary)]">2. 参数微调</div>
                                ...
                                {props.capabilityFields.map(field => {
                                    ...
                                    return (
                                        <div key={field.field} className="flex items-center justify-between gap-4">
                                            <span className="text-sm font-medium text-[var(--glass-text-primary)] shrink-0">{field.label || field.field}</span>
                                                ...
                                                {field.options.map(opt => {
                                                    ...
                                                    return (
                                                        <button
                                                            key={s}
                                                            onClick={() => props.onCapabilityChange(field.field, s, field.options[0])}
                                                            className={`px-3 py-1 text-sm transition-all rounded-md ${active ? 'bg-[var(--glass-text-primary)] text-[var(--glass-bg-base)] shadow-md font-bold' : 'bg-[var(--glass-bg-surface-strong)] text-[var(--glass-text-secondary)] hover:bg-[var(--glass-bg-hover)]'}`}
                                                        >
                                                            {s}
                                                        </button>
                                                    )
    ...
    return (
        <>
            ...
            >
                ...
                <div className="text-sm font-bold text-blue-500 bg-blue-500/10 px-3 py-1 rounded-full uppercase tracking-widest">
                    编辑
                </div>
            </button>
                    ...
                    <div className="relative w-full max-w-3xl h-[500px] flex rounded-2xl bg-[var(--glass-bg-base)] border border-[var(--glass-stroke-subtle)] shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-200">
                        ...
                        <div className="w-1/2 flex flex-col border-r border-[var(--glass-stroke-subtle)] bg-[var(--glass-bg-surface)]">
                            <div className="p-5 border-b border-[var(--glass-stroke-subtle)] flex items-center justify-between">
                                ...
                                <span className="text-sm text-[var(--glass-text-tertiary)]">包含 {props.models.length} 项</span>
                            </div>
                                ...
                                {props.models.map(m => {
                                    ...
                                    return (
                                        ...
                                        >
                                            ...
                                            {m.providerName && <div className={`text-sm mt-1 ${active ? 'text-blue-100' : 'text-[var(--glass-text-tertiary)]'}`}>{m.providerName}</div>}
                                        </button>
                                    )
                                })}
                            </div>
            ...
            >
                <div className="flex items-center gap-3">
                    ...
                    <div className="flex flex-col text-left">
                        <span className="text-sm font-semibold text-[var(--glass-text-primary)]">{activeModel ? activeModel.label : '未选择'}</span>
                        <span className="text-xs text-[var(--glass-text-tertiary)]">{props.capabilityFields.length} 项参数配置可设</span>
                    </div>
                </div>
                ...
                <div ref={panelRef} style={panelStyle} className="glass-surface-modal rounded-xl shadow-xl border border-[var(--glass-stroke-subtle)] overflow-hidden bg-[var(--glass-bg-base)]">
                    <div className={`flex w-[200%] transition-transform duration-300 ease-[cubic-bezier(0.32,0.72,0,1)] ${view === 'params' ? '-translate-x-1/2' : 'translate-x-0'}`}>
                        ...
                        <div className="w-1/2 flex flex-col max-h-[300px]">
                            <div className="p-3 border-b border-[var(--glass-stroke-subtle)] bg-[var(--glass-bg-surface)] font-bold text-sm text-center">选择主要模型</div>
                        ...
                        <div className="w-1/2 flex flex-col max-h-[300px]">
                            <div className="p-2 border-b border-[var(--glass-stroke-subtle)] bg-[var(--glass-bg-surface)] flex items-center">
                                <button onClick={() => setView('models')} className="p-1 px-2 shrink-0 flex items-center gap-1 hover:bg-[var(--glass-bg-hover)] rounded font-medium text-sm text-[var(--glass-text-secondary)]">
                                    <AppIcon name="chevronDown" className="w-4 h-4 rotate-90" />
                                    返回
                                </button>
                                <div className="font-bold text-sm text-center flex-1 mr-8">参数配置</div>
                            </div>
                                ...
                                {props.capabilityFields.map(field => {
                                    ...
                                    return (
                                        <div key={field.field} className="space-y-2">
                                            <div className="text-sm font-semibold text-[var(--glass-text-secondary)]">{field.label || field.field}</div>
                                                ...
                                                {field.options.map(opt => {
                                                    ...
                                                    return (
                                                        <button
                                                            key={s}
                                                            onClick={() => props.onCapabilityChange(field.field, s, field.options[0])}
                                                            className={`w-full p-2 text-sm text-center rounded-md border transition-all ${active ? 'bg-[#ff6b6b] text-white border-[#ff6b6b]' : 'bg-[var(--glass-bg-surface-strong)] border-[var(--glass-stroke-base)] text-[var(--glass-text-primary)] hover:border-[var(--glass-stroke-active)]'}`}
                                                        >
                                                            {s}
                                                        </button>
                                                    )
                                    ...
                                    {props.models.map(m => {
                                        ...
                                        return (
                                            ...
                                            >
                                                ...
                                                {m.providerName && <span className="text-xs font-medium text-[var(--glass-text-tertiary)] mt-1">{m.providerName}</span>}
                                            </button>
                                        )
                                    })}
                                </div>
