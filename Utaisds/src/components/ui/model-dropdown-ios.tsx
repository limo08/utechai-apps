...
function DefaultParamsRenderer({ fields, overrides, onChange, className }: { fields: CapabilityFieldDefinition[], overrides: Record<string, CapabilityValue>, onChange: (field: string, rawValue: string, sample: CapabilityValue) => void, className?: string }) {
    ...
    return (
        <div className={className}>
            <div className="text-xs font-bold text-[#8e8e93] px-1 pt-0.5 mb-2">参数配置</div>
            {fields.map(field => {
                ...
                if (field.field === 'duration' || field.options.length >= 4) {
                    return (
                        <div key={field.field} className="flex items-center justify-between gap-4 px-1 py-1 relative group">
                            <span className="text-sm font-semibold text-[var(--glass-text-secondary)] shrink-0">{field.label || field.field}</span>
                    ...
                    <div key={field.field} className="flex items-center justify-between gap-4 px-1 py-1">
                        ...
                        <div className="flex bg-[#f2f2f7] dark:bg-[#1c1c1e] p-[3px] rounded-lg shadow-inner">
                            {field.options.map(opt => {
                                ...
                                const active = s === val
                                return (
                                    <button key={s} onClick={() => onChange(field.field, s, field.options[0])} className={`px-4 py-1.5 text-sm font-medium rounded-md transition-all ${active ? 'bg-white text-black dark:bg-[#2c2c2e] dark:text-white shadow-[0_3px_8px_rgba(0,0,0,0.12),0_3px_1px_rgba(0,0,0,0.04)] font-bold' : 'text-[#8e8e93] hover:text-[#3a3a3c] dark:hover:text-[#ebebf5]'}`}>
                                        {s}
                                    </button>
                                )
                            })}
                ...
                <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                        ...
                        {activeModel?.providerName && (
                            <span className="text-xs font-medium px-2 py-0.5 rounded border border-[var(--glass-stroke-base)] text-[var(--glass-text-secondary)] whitespace-nowrap">
                                {activeModel.providerName}
                            </span>
                        )}
                    </div>
                    <div className="flex items-center gap-2">
                        <span className="text-sm text-[var(--glass-text-secondary)]">{resolveParamSummary(props.capabilityFields, props.capabilityOverrides)}</span>
