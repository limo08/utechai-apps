...
import type { CustomModel, Provider, AvailableModelData } from '../../api-config'
  ...
  const enabledModelsByType = useMemo(() => {
    ...
    if (availableModels && availableModels.length > 0) {
      ...
      for (const am of availableModels) {
        const groupKey = DISPLAY_TYPE_TO_GROUP[am.modelType]
        if (!groupKey) continue

        // 按 modelKey 去重（可能与本地模型重复）
        if (grouped[groupKey].some((m) => m.modelKey === am.modelKey)) continue

        grouped[groupKey].push({
          modelId: am.modelId,
          modelKey: am.modelKey,
          name: am.name,
          type: am.unifiedType as CustomModel['type'],
          provider: am.provider,
          providerName: am.ownedBy || 'Model Gateway',
          enabled: true,
          price: 0,
          priceLabel: '--',
        })
