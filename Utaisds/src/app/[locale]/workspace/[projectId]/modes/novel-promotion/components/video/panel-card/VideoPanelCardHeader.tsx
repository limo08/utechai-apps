  ...
  return (
    <div className="bg-[var(--glass-bg-muted)] flex items-center justify-center relative" style={{ aspectRatio: player.cssAspectRatio }}>
      ...
      {panel.lipSyncVideoUrl && hasVisibleBaseVideo ? (
        ...
        >
          <div className={`px-2 py-0.5 rounded-full text-xs font-medium transition-all ${!media.showLipSyncVideo ? 'bg-[var(--glass-tone-success-fg)] text-white' : 'text-[var(--glass-text-tertiary)] hover:text-white'}`}>
            {t('panelCard.original')}
          </div>
          <div className={`px-2 py-0.5 rounded-full text-xs font-medium transition-all ${media.showLipSyncVideo ? 'bg-[var(--glass-accent-from)] text-white' : 'text-[var(--glass-text-tertiary)] hover:text-white'}`}>
            {t('panelCard.synced')}
          </div>
        </div>
      ) : null}
