...
export const GET = apiHandler(async () => {
  ...
  // 从 AvailableModel 表读取网关持久化的可用模型（供"默认模型配置"下拉框使用）
  ...
  return NextResponse.json({
    ...
    workflowConcurrency,
    pricingDisplay,
    // 网关持久化的可用模型（按 7 种类型分组，供"默认模型配置"下拉框使用）
    availableModels: availableModels.map((m) => ({
      ...
      tags: m.tags ? safeParseTags(m.tags) : [],
      provider: 'gateway',
    })),
  })
})
