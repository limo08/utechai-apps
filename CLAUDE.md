...
- **waoowaoo/** — AI 影视 Studio (短剧/漫画视频制作工具)。Next.js 15 + React 19 + Prisma + BullMQ。有自己的 `.git` 目录，独立于父仓库管理。npm only。有完整的测试套件和大量 guard scripts。

## Conventions by Project

| | ClaudeCode / OpenClaw | waoowaoo |
|---|---|---|
...
### waoowaoo (from within `waoowaoo/` subdirectory)
...
- `waoowaoo/CLAUDE.md` — Full architecture: multi-process runtime, model gateway, task system, storage abstraction, billing, guard scripts, i18n